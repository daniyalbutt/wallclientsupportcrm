                                ...
                                @foreach($img_arr as $key => $presignedUrl)
                                    @if($client_files->id == $key)
                                        <tr>
                                            ...
                                            <td><button class="btn btn-primary btn-sm">{{$client_files->created_at->format('d M, y H:i:s') }}</span></td>
                                            <td>
...
<script>
    ...
    $(document).ready(function(){
        ...
        $("#image-file").on('fileuploaderror', function (event, data, msg) {
            toastr.error('File Upload Failed Due To Slow Internet', '', { timeOut: 5000 });
        });
    });
    var start = new Date;
