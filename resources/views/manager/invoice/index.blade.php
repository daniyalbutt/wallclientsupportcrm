...
<div class="row">
    <div class="col-md-12">
        <div class="card text-left">
            <div class="card-header p-0 position-relative mt-n4 mx-3 z-index-2">
              <div class="bg-gradient-dark shadow-dark border-radius-lg pt-4 pb-3">
                <h6 class="text-white text-capitalize ps-3" style="font-size: 1.75rem;">Invoice Details</h6>
              </div>
            </div>
                ...
                <table class="table align-items-center mb-0">
                  <thead>
                      <tr>
                        ...
                        <!--@if(Auth::user()->email == "development_manager@gmail.com" || Auth::user()->email == "james@projectwall.net" || Auth::user()->email == "adam_mackenzie@projectwall.net" || Auth::user()->email == "joseph@projectwall.net" || Auth::user()->email == "joe@projectwall.net" || Auth::user()->email == "chase@thenativepublishers.com")-->
                        <!--<th  class="text-center text-uppercase text-secondary text-xxs font-weight-bolder opacity-7">Send Mail</th>-->
                        <!--@endif-->
                        <!--<th  class="text-center text-uppercase text-secondary text-xxs font-weight-bolder opacity-7">Create Login</th>-->
                      ...
                      @foreach($data as $datas)
                        <tr>
                            ...
                            <td class="align-middle">
                                <div class="d-flex px-2 py-1" style="justify-content: center;text-align: center;">
                                  <div class="d-flex flex-column justify-content-center">
                                <span class="btn btn-primary btn-sm">{{ $datas->brands->name }}</span>
                                </div>
                                </div>
                            </td>
                            <td class="align-middle">
                            ...
                            <!--    <div class="d-flex px-2 py-1" style="justify-content: center;text-align: center;">-->
                            <!--      <div class="d-flex flex-column justify-content-center">-->
                            <!--    <button class="btn btn-glow btn-sm btn-warning send-invoice" data-id="{{ $datas->id }}" data-invoice-number="{{ $datas->invoice_number }}">Send Invoice</button>-->
                            <!--      </div>-->
                            <!--    </div>-->
                            <!--</td>-->
                            <!--@endif-->
                            ...
                            <td class="align-middle">
                                <div class="d-flex px-2 py-1" style="justify-content: center;text-align: center;">
                                  <div class="d-flex flex-column justify-content-center">
                                    ...
                                    @if($datas->payment_status == 1)
                                    <a href="{{ route('manager.invoice.edit', $datas->id) }}" class="mb-2 btn btn-glow btn-primary btn-icon btn-sm mr-1">
                                        <span class="ul-btn__icon"><i class="i-Edit"></i></span>
                                        <span class="ul-btn__text">Edit</span>
                                    </a>
                                    @endif
                                    <a href="{{ route('manager.link', $datas->id) }}" class="mb-2 btn btn-glow btn-info btn-icon btn-sm">
            ...
            <!--                            <form method="post" action="{{ route('manager.invoice.paid', $datas->id) }}">-->
            <!--                                @csrf-->
            <!--                                <button type="submit" class="mark-paid btn btn-danger p-0 ml-1">Click</button>-->
            <!--                            </form>-->
            <!--                            @endif-->
            <!--                        </span>-->
            <!--                    </td>-->
            <!--                    <td>{{ $datas->currency_show->sign }}{{ $datas->amount }}</td>-->
            <!--                    <td>{{ $datas->merchant->name }}</td>-->
            <!--                    <td>-->
            <!--                        <button class="btn btn-sm btn-secondary mb-1">{{ date('g:i a', strtotime($datas->created_at)) }}</button>-->
            <!--                        <button class="btn btn-sm btn-secondary">{{ date('d M, Y', strtotime($datas->created_at)) }}</button>-->
            <!--                    </td>-->
            <!--                    @if(Auth::user()->email == "development_manager@gmail.com" || Auth::user()->email == "james@projectwall.net" || Auth::user()->email == "adam_mackenzie@projectwall.net" || Auth::user()->email == "joseph@projectwall.net" || Auth::user()->email == "joe@projectwall.net" || Auth::user()->email == "chase@thenativepublishers.com")-->
            <!--                    <td>-->
            <!--                        <button class="btn btn-sm btn-warning send-invoice" data-id="{{ $datas->id }}" data-invoice-number="{{ $datas->invoice_number }}">Send Invoice</button>-->
            <!--                    </td>-->
            <!--                    @endif-->
                                <!--<td>                                    -->
                                <!--    @if($datas->payment_status == 2)-->
            ...
            <!--                    <td>-->
            <!--                        <div>-->
            <!--                            <a href="javascript:;" class="mb-2 btn btn-primary btn-icon btn-sm mr-1 more-detail" data-id="{{ $datas->id }}">-->
            <!--                                <span class="ul-btn__icon"><i class="i-Memory-Card-2"></i></span>-->
            <!--                                <span class="ul-btn__text">Details</span>    -->
            <!--                                <span class="badge badge-danger" id="flashing">New</span>-->
            <!--                            </a>-->
            ...
            <!--                            </a>-->
            <!--                            @endif-->
            <!--                            <a href="{{ route('manager.link', $datas->id) }}" class="mb-2 btn btn-info btn-icon btn-sm">-->
            <!--                                <span class="ul-btn__icon"><i class="i-Eye-Visible"></i></span>-->
            <!--                                <span class="ul-btn__text">View</span>-->
            <!--                            </a>-->
            <!--                        </div>-->
  ...
  <div class="modal-dialog modal-lg">
    <div class="modal-content">
      <div class="modal-header">
        ...
        <button type="button" class="btn-close btn btn-primary" data-bs-dismiss="modal" aria-label="Close" style="font-size: 22px;background: none;color: #fff;border: none;"><i class="i-Close"></i></button>
      </div>
...
<script>
    $('.send-invoice').on('click', function(){
        var id = $(this).data('id');
        var inv_num = $(this).data('invoice-number');
        ...
        $.ajax({
            type:'POST',
            url: "{{ route('manager.invoice.mail') }}",
            data: {id: id},
            success:function(data) {
                if(data.success == true){
                    swal("Invoice Mailed Successfully!", "Invoice# " + inv_num , "success");
                }else{
                    return swal({
                        title:"Error",
                        text: "There is an Error, Plase Contact Administrator",
...
$('.btn-close').on('click', function(){
    ...
    $('.more-detail').on('click', function(){
        ...
        $.ajax({
            type:'POST',
            url: "{{ route('manager.invoice.details') }}",
            data: {id: inv_id},
            success:function(data) {
                ...
                if (data.payment_status == 2) {
                    ...
                    actionButton = `
                        <a href="javascript:;" class="btn btn-glow btn-${btnClass} btn-sm auth-create" 
