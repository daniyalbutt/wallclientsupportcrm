            ...
            <div class="card-body">
                ...
                <form class="form" action="{{route('brand.update',$data->id)}}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-4 form-group mb-3">
                            <label for="zendesk">Zendesk Widget Script Key<span>*</span></label>
                            <input type="text" id="zendesk" class="form-control" placeholder="Zendesk Widget Script Key" name="zendesk" value="{{$data->zendesk}}">
                        </div>
                        <div class="col-md-4 form-group mb-3">
                            <label for="sign">Amount Sign<span>*</span></label>
                            <select name="sign" id="sign" class="form-control" required>
                                @foreach($currencies as $currency)
                                    <option value="{{$currency->sign}}">{{$currency->sign}} - {{$currency->name}}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="col-md-12">
