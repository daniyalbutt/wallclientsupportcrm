...
use App\Http\Controllers\ChargeCustomerController;
...
// Create ZIP FILE routes
Route::post('/download', [TaskController::class, 'download'])->name('download');
// Forms Zip File Routes
// Route::post('/download-webform', [WebFormController::class, 'download'])->name('download.webform');
// Route::post('/download-logoform', [LogoFormController::class, 'download'])->name('download.logoform');
// Route::post('/download-smmform', [SmmFormController::class, 'download'])->name('download.smmform');
// Route::post('/download-seoform', [SeoFormController::class, 'download'])->name('download.seoform');
// Route::post('/download-contentwritingform', [ContentWritingFormController::class, 'download'])->name('download.contentwritingform');
// Route::post('/download-bookformattingform', [BookFormattingController::class, 'download'])->name('download.bookformattingform');
// Route::post('/download-bookproofreadingform', [ProofreadingController::class, 'download'])->name('download.bookproofreadingform');
// Route::post('/download-bookwritingform', [BookWritingController::class, 'download'])->name('download.bookwritingform');
// Route::post('/download-authorwebform', [AuthorWebsiteController::class, 'download'])->name('download.authorwebform');
// Route::post('/download-coverdesignform', [BookCoverController::class, 'download'])->name('download.coverdesignform');
// Remove temporary zip
Route::post('/remove-zip', [TaskController::class, 'RemoveZipFile'])->name('zip.remove');
...
Route::group(['middleware' => 'auth'], function () {
    Route::group(['prefix' => 'admin',  'middleware' => 'is_admin'], function(){
        Route::get('/brand-leads',[BrandLeadsController::class, 'index'])->name('brand_leads.index');
        Route::get('/affiliate-leads',[BrandLeadsController::class, 'indexAffiliate'])->name('brand_leads.indexAffiliate');
        Route::get('/get-invoice-data', [AdminController::class, 'getInvoiceData'])->name('admin.invoice.chart');
        Route::post('/invoice-details/', [InvoiceController::class, 'invoiceDetail'])->name('admin.invoice.details');
        
        Route::get('home', [AdminController::class, 'dashboard'])->name('admin.home');
        Route::post('/invoices-revenue', [AdminController::class, 'revenueInvoices'])->name('revenue.invoices');
        ...
        Route::post('admin/file/delete', [AdminTaskController::class, 'deleteFilesAdmin'])->name('delete.files.admin');
        Route::post('admin-files/{id}', [TaskController::class, 'insertFiles'])->name('insert.admin.files');
        ...
        Route::get('invoice/{invoice}/pdf', [InvoiceController::class, 'covertToPdf'])->name('admin.invoice.pdf');
        ...
        Route::get('/{form_id}/projects/{check}/form/{id}', [SupportController::class, 'getFormAdmin'])->name('admin.form');
        Route::get('settings/project-settings', [AdminController::class, 'projectSettings'])->name('admin.project.settings');
        Route::post('settings/project-settings-update', [AdminController::class, 'updateProjectSettings'])->name('admin.project.settings.update');
        
        //Lead Generation Routes
        Route::get('/leads', [LeadGenerationController::class, 'index'])->name('admin.leads.index');
        Route::get('/leads/create', [LeadGenerationController::class, 'create'])->name('admin.leads.create');
        Route::post('/leads/store', [LeadGenerationController::class, 'store'])->name('admin.leads.store');
        Route::get('/get/brand/users', [LeadGenerationController::class, 'getUserBrands'])->name('admin.user.brand');
        Route::get('/leads/edit/{id}', [LeadGenerationController::class, 'edit'])->name('admin.leads.edit');
        Route::post('/leads/update/{id}', [LeadGenerationController::class, 'update'])->name('admin.leads.update');
        Route::post('/leads/update/status/{id}', [LeadGenerationController::class, 'updateStatus'])->name('admin.leads.update.status');
        Route::get('/leads/delete/{id}', [LeadGenerationController::class, 'destroy'])->name('admin.leads.delete');
        Route::get('/get/lead/users', [LeadGenerationController::class, 'get_assign_user'])->name('admin.get.agent');
        Route::post('/assign/lead', [LeadGenerationController::class, 'assign_lead'])->name('admin.assign.agent');
        Route::post('/remove/user', [LeadGenerationController::class, 'removeAgentManager'])->name('admin.remove.user');
        Route::post('/manager-access', [LeadGenerationController::class, 'enableManagerAssign'])->name('admin.manager.access');
        Route::get('/lead/logs', [LeadGenerationController::class, 'LeadLogs'])->name('admin.lead.logs');
        Route::post('/leads/{id}/update-note', [LeadGenerationController::class, 'updateNotes'])->name('admin.update.note');
    });
});
...
Route::group(['middleware' => 'auth'], function () {
    Route::group(['middleware' => 'is_support'], function(){
        ...
        Route::post('support/file/delete', [TaskController::class, 'deleteFiles'])->name('delete.files.support');
        ...
        // Create ZIP FILE routes
        Route::post('support/download-forms', [SupportController::class, 'download'])->name('download.support.forms');
    });
});
...
Route::group(['middleware' => 'auth'], function () {
    Route::group(['middleware' => 'is_sale'], function(){
        ...
        // Lead Generation
        Route::get('/sale/leads', [LeadGenerationController::class, 'index'])->name('sale.leads.index');
        Route::post('/sale/leads/update/status/{id}', [LeadGenerationController::class, 'updateStatus'])->name('sale.update.status');
        Route::post('/sale/lead-viewed', [LeadGenerationController::class, 'dataViewHistory'])->name('sale.lead.view');
        Route::post('/sale/leads/{id}/update-note', [LeadGenerationController::class, 'updateNotes'])->name('sale.update.note');
    });
});
Route::group(['middleware' => 'auth'], function () {
    Route::group(['middleware' => 'is_production'], function(){
        ...
        Route::post('production/download-forms', [TaskController::class, 'formDownload'])->name('download.production.forms');
        Route::post('/production/theme', [HomeController::class, 'updateTheme'])->name('production.theme');
    });
});
Route::group(['middleware' => 'auth'], function () {
    Route::group(['middleware' => 'is_member'], function(){
        ...
        Route::post('/member/file/delete', [TaskController::class, 'deleteFiles'])->name('delete.files.member');
    });
});
...
Route::group(['middleware' => 'auth'], function () {
    Route::group(['middleware' => 'is_sale_manager'], function(){
        ...
        Route::get('invoice/{invoice}/pdf', [InvoiceController::class, 'covertToPdf'])->name('manager.invoice.pdf');
        Route::post('invoice/mail' , [InvoiceController::class, 'sendMailInvoice'])->name('manager.invoice.mail');
        
        
        Route::post('/manager/invoice-details/', [InvoiceController::class, 'invoiceDetail'])->name('manager.invoice.details');
        Route::get('/manager/my-invoice/', [InvoiceController::class, 'myInvoiceManager'])->name('manager.my.invoices');
        
        
        //Lead Generation Routes
        Route::get('/leads', [LeadGenerationController::class, 'index'])->name('manager.leads.index');
        Route::post('/leads/update/status/{id}', [LeadGenerationController::class, 'updateStatus'])->name('manager.update.status');
        Route::get('/get/lead/users', [LeadGenerationController::class, 'get_agent'])->name('manager.get.agent');
        Route::post('/assign/lead', [LeadGenerationController::class, 'assign_lead'])->name('manager.assign.agent');
        Route::post('/lead-viewed', [LeadGenerationController::class, 'dataViewHistory'])->name('manager.lead.view');
        Route::post('/leads/{id}/update-note', [LeadGenerationController::class, 'updateNotes'])->name('manager.update.note');
        
        // Charge Authorize Account
        Route::get('/charge-customer', [ChargeCustomerController::class, 'index'])->name('manager.authorize.index');
        Route::post('/get-transaction-details', [ChargeCustomerController::class, 'getTransactionDetails'])->name('manager.transaction.details');
        Route::post('/charge-customer', [ChargeCustomerController::class, 'charge'])->name('manager.authorize.charge');
    });
});
