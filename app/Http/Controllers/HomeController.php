...
class HomeController extends Controller
{
    ...
    public function managerDashboard(){
        ...
        // Paid Invoice Count
        $paid_invoice = DB::table('invoices')->where('payment_status', 2)->where('sales_agent_id', Auth::user()->id)->whereIn('brand', Auth::user()->brand_list())->count();
        // Unpaid Invoice Count
        $un_paid_invoice = DB::table('invoices')->where('payment_status', 1)->where('sales_agent_id', Auth::user()->id)->whereIn('brand', Auth::user()->brand_list())->count();
        // My Invoices Count
        $my_invoice = DB::table('invoices')->where('sales_agent_id',Auth::user()->id)->count();
        //Paid Invoice Total
        $paid_invoice_total = number_format(DB::table('invoices')->where('payment_status', 2)->where('sales_agent_id', Auth::user()->id)->whereIn('brand', Auth::user()->brand_list())->sum('amount'));
        //Unpaid Invoice Total
        $unpaid_invoice_total = number_format(DB::table('invoices')->where('payment_status', 1)->where('sales_agent_id', Auth::user()->id)->whereIn('brand', Auth::user()->brand_list())->sum('amount'));
        // Get Recent Clients
        $recent_clients = Client::with('brand','user','added_by','agent')->whereIn('brand_id', Auth::user()->brand_list())->orderBy('id', 'desc')->limit(6)->get();
        
        return view('manager.home', compact('clients_count', 'paid_invoice', 'un_paid_invoice','my_invoice','paid_invoice_total','unpaid_invoice_total','recent_clients'));
    }
